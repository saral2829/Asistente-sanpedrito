  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("' + data.fetch.avatar + '")');
        $(".header-avatar").css(
          "background-image",
          'url("' + data.fetch.avatar + '")'
        );
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        if (data.error > 0) {
          ...
          //si es mensaje a bot cargar respuesta en otro ajax
          if(data.msje_bot){
            //Si el mensaje es para el bot este responde 
            bot_responde(data,data.msj);
          }
        }
      },
      error: () => {
...
function bot_responde(json_data,msjeee){
  temporaryMsgId += 1;
  let tempID = "temp_" + temporaryMsgId;
  let j_data = new FormData();
  //json_data.append("_token", access_token);
  j_data.append('_token',access_token);
  j_data.append('msj',msjeee);
  /*json_data["temporaryMsgId"] = tempID;
  json_data["_token"] = access_token;
  json_data['msj'] = msjeee;*/
  $.ajax({
    url: $("#message-form").attr("action").replace('sendMessage','replyBot'),
    method: "POST",
    data:j_data,
    dataType: "JSON",
    processData: false,
